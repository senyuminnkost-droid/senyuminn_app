              ...
              return (
                <div key={it.id} style={{ display:"grid", gridTemplateColumns:"130px 1fr 110px 28px", gap:6, marginBottom:6, alignItems:"center" }}>
                  ...
                  <button onClick={()=>delItem(idx)} disabled={items.length===1}
                    style={{ width:24, height:24, borderRadius:6, border:"1px solid var(--s200)", background:"var(--s50)", cursor:items.length===1?"not-allowed":"pointer", color:"var(--s400)", fontSize:12, display:"flex", alignItems:"center", justifyContent:"center", opacity:items.length===1 ? 0.3 : 1 }}>×</button>
                </div>
              );
            })}
