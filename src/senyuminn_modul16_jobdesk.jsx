                  ...
                  {shiftData.map(item=>{
                    ...
                    return (
                      <button key={item.id} onClick={()=>toggleKlaim(item.id, st.id)}
                        style={{ padding:"4px 10px", borderRadius:7, border:`1.5px solid ${claimed?"#7c3aed":"var(--s200)"}`, background:claimed?"#f3e8ff":"var(--white)", color:claimed?"#7c3aed":"var(--s500)", fontSize:11, fontWeight:claimed?700:400, cursor:"pointer", fontFamily:"inherit", textDecoration:done(item.id)?"line-through":"none", opacity:done(item.id) ? 0.5 : 1 }}>
